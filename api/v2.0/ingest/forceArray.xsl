        ...
        <xsl:choose>
            <xsl:when test="tei:ref[@type='cmif:hasLanguage' or @type='cmif:fulltext']">
                <note xmlns="http://www.tei-c.org/ns/1.0">
                    <xsl:for-each select="tei:ref[@type='cmif:hasLanguage' or @type='https://lod.academy/cmif/vocab/terms#hasLanguage']">
                        <xsl:variable name="language">
                            <xsl:choose>
                                <xsl:when test="matches(./@target, 'https://tools.ietf.org/html/bcp47/')">
                                    <xsl:value-of select="substring-after(@target, 'bcp47/')"/>
                                </xsl:when>
                                <xsl:otherwise>
                        ...
                        <tei:ref type="https://lod.academy/cmif/ns/terms#hasLanguage" target="{$language}" json:array="true"><xsl:value-of select="."/></tei:ref>
                    </xsl:for-each>
