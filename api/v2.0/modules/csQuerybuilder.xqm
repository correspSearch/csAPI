...
module namespace csQb="https://correspsearch.net/api/query-builder";
    ...
    map {
        "nested": map {
            ...
            "query" : map {
                "bool" : map {
                    "must" : 
                        csQb:tokenize-single-s-p-o($param-type, $param-part)            
...
declare function csQb:build-query($aggregation as xs:boolean, $size as xs:int, $page as xs:int, $cmif-url as xs:string?) {
...
let $cmif :=
    if ($cmif-url)
    then csQb:subquery-e-c-a('cmif_idno', $cmif-url, 'should')
    else if ($csQb:p_c)
